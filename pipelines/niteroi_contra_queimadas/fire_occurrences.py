...
def update_df():
    ...
    try:
        ...
        raise ValueError(f"Erro ao preparar dataframe: {e}")

@task
def agol_connections():
    try:
        #Conectando ao Agol
...
def execute(df):    
    try:
        ...
        raise ValueError(f"Erro ao executar o fluxo para ocorrências de incêndio: {e}")

# Função main para rodar o código
@flow(name="Fluxo ocorrências de incêndio")
def fire_occurences_flow():
    try:
        df = update_df()
        execute(df)
    except requests.exceptions.RequestException as e:
