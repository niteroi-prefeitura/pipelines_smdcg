...
LAYER_NAME_PORTAL_HIST_PLUV = os.getenv("LAYER_NAME_PORTAL_HIST_PLUV") or svida_variables["LAYER_NAME_PORTAL_HIST_PLUV"]
LAYER_ID_AGOL_SVIDA = os.getenv("LAYER_ID_AGOL_SVIDA") or svida_variables["LAYER_ID_AGOL_SVIDA"]


URL_PREVISAO_API = os.getenv("URL_PREVISAO_API") or svida_variables["URL_PREVISAO_API"]
URL_PONTOS_APOIO_API = os.getenv("URL_PONTOS_APOIO_API") or svida_variables["URL_PONTOS_APOIO_API"]
URL_PLUVIOMETROS_API = os.getenv("URL_PLUVIOMETROS_API") or svida_variables["URL_PLUVIOMETROS_API"]
URL_ESTAGIO_ATENC_API = os.getenv("URL_ESTAGIO_ATENC_API") or svida_variables["URL_ESTAGIO_ATENC_API"]
URL_SIRENES_API = os.getenv("URL_SIRENES_API") or svida_variables["URL_SIRENES_API"]
URL_METEOROLOGIA_API = os.getenv("URL_METEOROLOGIA_API") or svida_variables["URL_METEOROLOGIA_API"]
URL_AVISOS_API = os.getenv("URL_AVISOS_API") or svida_variables["URL_AVISOS_API"]
...
def weather_preview():
    try:
        ...
        raise ValueError(f"Erro weather preview: {e}")
...
def support_places():
    try:
        ...
        raise ValueError(f"Erro support places: {e}")
...
def pluv_treatment():
    ...
    try:      
        ...
        raise ValueError(f"Erro pluv treatment: {e}")
...
def portal_conection():
    try:
        ...
        raise ValueError(f"Erro ao tentar conectar ao portal: {e}")
...
def attention_stage(now):
    try:        
        ...
        raise ValueError(f"Erro attention stage: {e}")
...
def sirens():
    try:        
        ...
        raise ValueError(f"Erro sirens: {e}")
...
def meteorology():
    try:
        ...
        raise ValueError(f"Erro meteorology: {e}")
...
def alerts():
    try:
        ...
        raise ValueError(f"Erro alerts: {e}")
...
def gis_function(df_pluv, df_attention_stage, df_support_place, df_weather_preview, df_sirens, df_meteoro, df_alerts, now, pluv_hist):
    ...
    # Atualizando os dados de pluviômetros com `update_pluv`
        ...
        raise ValueError(f"Erro gis function: {e}")
